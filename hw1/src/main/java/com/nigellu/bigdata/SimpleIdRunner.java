...
public class SimpleIdRunner {
  ...
  public static void run(String inputPath, String outputPath, Configuration config, Log LOG) throws Exception {
    ...
    Job job2 = Job.getInstance(config, "simple id");
    job2.setJarByClass(TopTenRunner.class);
    job2.setMapperClass(SimpleIdMapperReducer.SimpleIdMapper.class);
    job2.setReducerClass(SimpleIdMapperReducer.SimpleIdReducer.class);
    job2.setMapOutputKeyClass(IntWritable.class);
    job2.setMapOutputValueClass(Text.class);
    job2.setOutputKeyClass(IntWritable.class);
    job2.setOutputValueClass(Text.class);
    // * Use the self-defined comparator for descending order sorting
    job2.setSortComparatorClass(SimpleIdMapperReducer.DescendingIntComparator.class);
